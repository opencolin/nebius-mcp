...
| 3 | HTTP client and the Token Factory credential axis | M | `httpx` added to dependencies; `auth.resolve_token_factory_credentials()` reads `NEBIUS_TOKEN_FACTORY_API_KEY` then `NEBIUS_API_KEY` then the profile; `check_environment` reports both planes side by side; a 401 surfaces as a typed error, never an httpx traceback. |
| 4 | Model catalog and base-URL resolution | S | Default base URL is the generic superset host, overridable; a unit test asserts the string `api.studio.nebius.com` appears nowhere under `src/`; `tokenfactory_resolve_base_url` warns that regional hosts serve subsets. |
...
| 6 | Token Factory files and fine-tuning | L | Nine tools; delete and cancel go through `preview_or_execute`; every endpoint path confirmed against a live tenant and recorded in `docs/token-factory-api.md` with its verification date; an unconfirmable path ships as a tool that says so rather than a 404 traceback. |
...
| 6 | Sandboxes: code execution as a first-class surface | L | Mandatory TTL with a hard cap and `expires_at` on list; output capped and wrapped; write-mode gated and audited with an argument hash; a live test executes, asserts output, deletes, and asserts a subsequent get is not-found. |
